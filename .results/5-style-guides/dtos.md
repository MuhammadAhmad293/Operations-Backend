...
- `ResponseDto<EmptyResponseDto>` — for write operations (create/update/delete) where no data is returned.
...
Write operations use `EmptyResponseDto` as the `T` in `ResponseDto<T>` to indicate no data payload:
```csharp
