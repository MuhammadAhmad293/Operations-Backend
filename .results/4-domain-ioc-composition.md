...
|--------------------------|---------------------------------------------|----------------------------------------------------|
| `CoreServicesResolver`   | `Operations.Services/Resolver/`             | Services (Scoped), Localization (Scoped), Mapster  |
| `UnitOfWorkResolver`     | `Operations.Repositories/Resolver/`         | DbContext, IUnitOfWork (Scoped), Lazy<> helper      |
| `CommonResolver`         | `Common/Resolver/`                          | Mail, File, Validator, HttpClient (all Scoped)     |
| `IocManager`             | `Operations.Ioc/`                           | Aggregates Repo resolvers (thin wrapper, optional) |
...
`Assembly.GetExecutingAssembly()` targets `Operations.Services`, so all `IRegister` classes in that project are auto-discovered.
