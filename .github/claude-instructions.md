# Claude Instructions — Meezan Backend
...
**Meezan Backend** is a .NET 10 ASP.NET Core Web API implementing Clean Architecture. It is a user account management + background job reference project. The domain is simple (user CRUD + email notification), but the infrastructure patterns are the important part to follow.
...
| Project                   | Role                                                     |
| ------------------------- | -------------------------------------------------------- |
| `Meezan`                  | API host: controllers, middleware, filters, Program.cs   |
| `Meezan.IServices`        | Service interfaces                                       |
| `Meezan.Services`         | Business logic, mappers, localization, custom exceptions |
| `Meezan.IRepositories`    | Repository and UnitOfWork interfaces                     |
| `Meezan.Repositories`     | EF Core DbContext, repositories, migrations, UoW impl    |
| `Meezan.DataModel`        | Entity models                                            |
| `Meezan.Dto`              | Domain DTOs (UserDto, etc.)                              |
| `Meezan.Ioc`              | IoC aggregation helper                                   |
...
**Examples:** `Meezan/Controllers/UserController.cs`, `Meezan/Controllers/JobTestController.cs`
**Conventions:**
...
**Examples:** `Meezan/ErrorHandlingMiddleware.cs`
**Conventions:**
...
**Examples:** `Meezan.Services/Auth/IJwtTokenGenerator.cs` / `JwtTokenGenerator.cs`, `Meezan.Services/AuthService/AuthService.cs`, `Meezan/Controllers/AuthController.cs`, `Meezan.DataModel/Entities/PasswordResetToken.cs`
**Conventions:**
...
**Examples:** `Meezan/Filter/SwaggerHeaderFilter.cs`
**Conventions:**
...
**Examples:** `Meezan.DataModel/Entities/User.cs`, `Meezan.DataModel/Entities/Mail.cs`
**Conventions:**
...
**Examples:** `Meezan.DataModel/Base/BaseEntity.cs`, `Meezan.DataModel/Base/BaseMultilingualTextEntity.cs`

### `dtos`

**Purpose:** Data transfer between API layer and service layer.
**Examples:** `Meezan.Dto/DTOs/UserDto.cs`, `Common/Dto/ResponseDto.cs`
**Conventions:**
...
**Examples:** `Meezan.IServices/IService/IUserService.cs`
**Conventions:**
...
**Examples:** `Meezan.Services/UserService/UserService.cs`
**Conventions:**
...
**Examples:** `Meezan.Services/Base/BaseService.cs`

### `repositories`

**Purpose:** Entity-specific data access.
**Examples:** `Meezan.Repositories/Repository/UserRepository.cs`
**Conventions:**

- Extend `BaseRepository<T>` — constructor only passes `Lazy<AppDbContext>` to base.
- Implement the matching `IBaseRepository<T>`-extending interface from `Meezan.IRepositories`.

### `base-repositories`

**Purpose:** Generic EF Core CRUD.
**Examples:** `Meezan.Repositories/Base/BaseRepository.cs`
**Conventions:**

- Write Meezan (`CreateAsyn`, `Update`, `Delete`) are synchronous enqueue — no await.
- Reads use `Expression<Func<T, bool>>` predicates.

### `repository-interfaces`

**Purpose:** Repository contracts.
**Examples:** `Meezan.IRepositories/IRepository/IUserRepository.cs`
**Conventions:**
...
**Examples:** `Meezan.IRepositories/UnitOfWork/IUnitOfWork.cs`, `Meezan.Repositories/UnitOfWork/UnitOfWork.cs`
**Conventions:**
...
**Examples:** `Meezan.Repositories/Context/AppDbContext.cs`
**Conventions:**
...
**Examples:** `Meezan.Repositories/EntityConfiguration/UserConfiguration.cs`
**Conventions:**
...
**Examples:** `Meezan.Repositories/Migrations/`
**Conventions:**
...
**Examples:** `Meezan.Services/Mapper/UserMapper.cs`
**Conventions:**
...
**Examples:** `Meezan.Services/Localization/LocalizationService.cs`, `localizationFile.json`
**Conventions:**
...
**Examples:** `Meezan.Services/Setting/MailSettings.cs`
**Conventions:**
...
**Examples:** `Common/Notification/Mail/MailSender.cs`, `Meezan.Services/Email/ISmtpEmailSender.cs` / `SmtpEmailSender.cs`
**Conventions:**
...
**Examples:** `Meezan.Services/Job/JobService.cs`
**Conventions:**
...
**Examples:** `Meezan.Services/Outbox/OutboxPublisherService.cs`, `Meezan.Services/Email/EmailConsumer.cs`, `Meezan.Services/Email/DeadLetterHandler.cs`, `Meezan.Services/Messaging/RabbitConnectionManager.cs`
**Conventions:**
...
**Examples:** `Meezan.Services/Messaging/RabbitConnectionManager.cs`, `IRabbitPublisher.cs` / `RabbitPublisher.cs`, `RabbitTopologyDeclarator.cs`
**Conventions:**
...
**Examples:** `Meezan.DataModel/Entities/OutboxMessage.cs`, `ProcessedMessage.cs`, `Meezan.Services/Outbox/OutboxPublisherService.cs`
**Conventions:**
...
**Examples:** `Meezan.Services/Email/EmailResiliencePipeline.cs`
**Conventions:**
...
**Examples:** `Meezan.Services/HealthChecks/RabbitMqHealthCheck.cs`, `OutboxBacklogHealthCheck.cs`
**Conventions:**
...
1. **Entity** — create `Meezan.DataModel/Entities/Product.cs` extending `BaseEntity`.
2. **Entity Configuration** — create `Meezan.Repositories/EntityConfiguration/ProductConfiguration.cs` (internal, `IEntityTypeConfiguration<Product>`) setting GETDATE defaults for audit columns.
3. **DbSet** — add `public DbSet<Product> Product { get; set; }` to `AppDbContext`.
4. **Repository Interface** — create `Meezan.IRepositories/IRepository/IProductRepository.cs` extending `IBaseRepository<Product>`.
5. **Repository** — create `Meezan.Repositories/Repository/ProductRepository.cs` extending `BaseRepository<Product>`.
...
7. **DTO** — create `Meezan.Dto/DTOs/ProductDto.cs` (plain POCO).
8. **Mapper** — create `Meezan.Services/Mapper/ProductMapper.cs` implementing `IRegister`.
9. **Service Interface** — create `Meezan.IServices/IService/IProductService.cs` returning `Task<ResponseDto<T>>`.
10. **Service** — create `Meezan.Services/ProductService/ProductService.cs` extending `BaseService`, implementing `IProductService`.
11. **Register Service** — add `services.AddScoped<IProductService, ProductService>()` in `CoreServicesResolver.ResolveCoreServices`.
12. **Controller** — create `Meezan/Controllers/ProductController.cs` extending `ControllerBase`.
13. **Migration** — run `dotnet ef migrations add AddProductEntity` in `Meezan.Repositories`.

### Adding a New Localizable String

1. Add entry to `Meezan.Services/Localization/LocalizationFileReader/localizationFile.json` (both `en` and `ar`).
2. Add property to `ILocalizationService`.
3. Implement in `LocalizationService` using `GetKeyValue("Key", "altValue")`.

### Adding a New Background Job

1. Add method signature to `Meezan.IServices/IJob/IJobService.cs`.
2. Implement in `Meezan.Services/Job/JobService.cs`.
3. Add controller action dispatching it via `IBackgroundJobClient` or `IRecurringJobManager`.

### Adding a New Custom Exception

1. Create `Meezan.Services/CustomExceptions/ProductNotFoundException.cs` with single-arg constructor.
...
- **Commit after staging** — stage all repository Meezan, then call `CommitAsync()` once.
...
- **SwaggerHeaderFilter adds Accept-Language automatically** — do not add it to individual Meezan.
...
**Feature request:** "Add a Product entity with CRUD Meezan."

**Files to generate:**

| File                                                              | Category              |
| ----------------------------------------------------------------- | --------------------- |
| `Meezan.DataModel/Entities/Product.cs`                            | entities              |
| `Meezan.Repositories/EntityConfiguration/ProductConfiguration.cs` | entity-configurations |
| `Meezan.IRepositories/IRepository/IProductRepository.cs`          | repository-interfaces |
| `Meezan.Repositories/Repository/ProductRepository.cs`             | repositories          |
| `Meezan.Dto/DTOs/ProductDto.cs`                                   | dtos                  |
| `Meezan.Services/Mapper/ProductMapper.cs`                         | mappers               |
| `Meezan.IServices/IService/IProductService.cs`                    | service-interfaces    |
| `Meezan.Services/ProductService/ProductService.cs`                | services              |
| `Meezan/Controllers/ProductController.cs`                         | api-controllers       |

**Files to modify:**

| File                                                                        | Change                                              |
| --------------------------------------------------------------------------- | --------------------------------------------------- |
| `Meezan.Repositories/Context/AppDbContext.cs`                               | Add `DbSet<Product>`                                |
| `Meezan.IRepositories/UnitOfWork/IUnitOfWork.cs`                            | Add `IProductRepository ProductRepository { get; }` |
| `Meezan.Repositories/UnitOfWork/UnitOfWork.cs`                              | Add repository property                             |
| `Meezan.Services/Resolver/CoreServicesResolver.cs`                          | Register `IProductService`                          |
| `Meezan.Services/Localization/LocalizationFileReader/localizationFile.json` | Add any new message keys                            |
...
Then run `dotnet ef migrations add AddProductEntity` in `Meezan.Repositories`.
